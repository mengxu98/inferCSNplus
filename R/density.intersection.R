...
density.points <- function(
    ...
    file_save = NULL,
    color_list = c(
      "#66cc33", "#66ff33", "#ccff00", "#ffff99", "#ffff33",
      "#ffcc33", "#ff9933", "#ff6633", "#cc3300", "#ff3333"
    )) {
    ...
    for (i in seq_len(nrow(points_data) - 1)) {
      results <- rbind(
        ...
        data.frame(
          ...
          time_point1 = points_data$max_density_pseudotime[i + 1]
        )
      )
    }
  }
...
dynamic.windowing <- function(
  ...
  for (i in seq_len(nrow(res))) {
    list_names1 <- c(list_names1, paste0(key_word, i, key_sign, key_word, i))
  }
  ...
  for (i in seq_len(nrow(bin_points))) {
    list_names2 <- c(list_names2, paste0(key_word, i, key_sign, key_word, i + 1))
  }

  cells_list2 <- apply(
    bin_points, 1, function(x) {
      meta_data_sub1 <- dplyr::filter(meta_data, cluster == x[1])
      meta_data_sub2 <- dplyr::filter(meta_data, cluster == x[2])
      meta_data_sub1 <- meta_data_sub1[which(meta_data_sub1$pseudotime > x[3]), ]
      meta_data_sub2 <- meta_data_sub2[which(meta_data_sub2$pseudotime <= x[4]), ]
      rbind(
        meta_data_sub1,
        meta_data_sub2
      )
