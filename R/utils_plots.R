...
plot_module_metrics.CSNObject <- function(
    object,
    network = DefaultNetwork(object),
    ...) {
...
get_network_graph.CSNObject <- function(
    object,
    network = DefaultNetwork(object),
  ...
  if (umap_method == "weighted") {
    ...
    gene_cor_df <- gene_cor %>%
      ...
      tidyr::pivot_longer(!source, names_to = "target", values_to = "corr")

    modules_use <- modules@meta %>%
      dplyr::filter(target %in% colnames(rna_expr), tf %in% colnames(rna_expr))
...
plot_network_graph.CSNObject <- function(
    object,
    network = DefaultNetwork(object),
    graph = "module_graph",
    layout = "umap",
...
get_tf_network.CSNObject <- function(
    object,
    tf,
    network = DefaultNetwork(object),
    graph = "module_graph",
...
plot_tf_network.CSNObject <- function(
    object,
    tf,
    network = DefaultNetwork(object),
    graph = "module_graph",
  ...
  p <- ggraph::ggraph(gene_graph, layout = "tree", circular = circular)
