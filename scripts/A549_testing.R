...
dataset <- "A549"
input_path <- paste0("../inferCSN_data/", dataset, "/data/")
output_path <- paste0("../inferCSN_data/", dataset, "/output/")
figures_path <- paste0("../inferCSN_data/", dataset, "/figures/")

rna.counts <- read.table(file = paste0(input_path, "A549_RNA_count.txt"))
atac.counts <- read.table(file = paste0(input_path, "A549_ATAC_count.txt"))
label <- read.table(paste0(input_path, "A549_time.txt"), sep = "\t")
...
markers <- read.table(file = paste0(input_path, "A549_DE_genes.txt"), header = TRUE, sep = "\t") # identified by Wilcox rank test
markers$gene <- as.character(markers$gene)
markers$group <- as.character(markers$group)

genome.info <- read.table(file = paste0(input_path, "hg19.promoter.regions.txt"))
...
combined <- Run_DIRECT_NET(combined,
                           k_neigh = 50,
                           atacbinary = TRUE,
                           max_overlap = 0.5,
                           size_factor_normalize = TRUE,
                           genome.info = genome.info,
                           focus_markers = c("PYGB","NR3C1"))
