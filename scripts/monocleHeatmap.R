globalVariables(c(
  "Cluster", "Pseudotime", "State", "buildBranchCellDataSet",
...
traverseTree <- function(g, starting_cell, end_cells) {
  ...
  return(list(
    shortest_path = path$vpath, distance = distance,
...
#' @export
plot_pseudotime_heatmap2 <- function(
    cds_subset,
    cluster_rows = TRUE,
  ...
  pseudocount <- 1
  # newdata <- data.frame(
  #   Pseudotime = seq(min(pData(cds_subset)$Pseudotime), max(pData(cds_subset)$Pseudotime), length.out = 100)
  # )
  newdata <- data.frame(
    Pseudotime = seq(min(meta_data$pseudotime), max(meta_data$pseudotime), length.out = 100)
  )
  m <- genSmoothCurves(
    cds_subset,
    cores = cores,
  ...
  ph_res <- pheatmap::pheatmap(
    heatmap_matrix[, ], # ph$tree_row$order
    useRaster = T,
    cluster_cols = FALSE,
    cluster_rows = cluster_rows,
  ...
  purrr::map_df(seq_along(id), function(x) {
    ...
    tmp %>%
      dplyr::mutate(
        cluster_name = paste(cluster_name, " (", cl.info$Freq[x], ")", sep = "")
      )
  }) -> df
...
#' @importFrom stats sd as.dist cor cutree
#' @export
plot_genes_branched_heatmap2 <- function(
    cds_subset = NULL,
    branch_point = 1,
    branch_states = NULL,
...
plot_multiple_branches_heatmap2 <- function(
  ...
  for (branch_in in branches) {
    ...
    if (cds@dim_reduce_type != "ICA") {
      root_state_cells <- unique(
        paste("Y_", cds@auxOrderingData$DDRTree$pr_graph_cell_proj_closest_vertex[root_state_cells, ], sep = "")
      )
      branches_cells <- unique(
        paste("Y_", cds@auxOrderingData$DDRTree$pr_graph_cell_proj_closest_vertex[branches_cells, ], sep = "")
      )
    }
  ...
  # TODO: FIXME: this needs to check that vst values can even be computed. (They can only be if we're using NB as the expressionFamily)
...
pre_pseudotime_matrix <- function(
  ...
  if (assays == "counts") {
    ...
    pt.matrix <- monocle3::normalized_counts(cds_obj, norm_method = "log")[
      match(gene_list, rownames(SummarizedExperiment::rowData(cds_obj))),
      order(monocle3::pseudotime(cds_obj))
    ]
  }
